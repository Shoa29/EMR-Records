...
require ("entities/documentEntity.php");
...
class userModel 
{
        ...
        function loginUser($username, $pwd)
    {
        ...
        Print '<script>window.location.assign("ulogin.php");</script>'; // redirects to login.php
    ...
    function  getDocumentsByUsername($username){
       ...
       //connection to database 
        ...
        mysqli_select_db($conn,$database) ; 
        //Perform Query
        $query = "SELECT * from documents WHERE userid='$username'";
        $result=mysqli_query($conn,$query) or die(mysqli_error($conn));
        //get data form database
       ...
       {   
           ...
           $document = new documentEntity($id, $username, $content, $temp1, $temp2);
    ...
    function sendNotification($latitude, $longitude,$userid ){
      ...
      $content= " There is an emergency case at Loction : latitude = ".$latitude." and Longitude = ".$longitude."  " ;
